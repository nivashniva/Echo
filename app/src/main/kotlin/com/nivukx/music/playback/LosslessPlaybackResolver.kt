...
class LosslessPlaybackResolver @Inject constructor(
    ...
    suspend fun resolve(videoId: String): Result<YTPlayerUtils.PlaybackData> =
        withContext(Dispatchers.IO) {
            runCatching {
                ...
                if (query.isBlank()) {
                    error("Cannot resolve lossless source without track metadata")
                }
    ...
    private fun matchRecording(
        ...
        candidates
            ...
            .map { candidate ->
                ...
                val artistScore =
                    similarity(normalize(artist), normalize(candidate.artist))
                val durationScore =
                    durationSeconds?.let { target ->
                ...
                candidate to
                    (titleScore * 0.55 + artistScore * 0.30 + durationScore * 0.15)
            }
    ...
    private fun similarity(a: String, b: String): Double {
        ...
        return (left.intersect(right).size.toDouble() * 2.0) /
    ...
    private fun normalize(value: String): String =
        value
            ...
            .replace(Regex("""\([^)]*\)|\[[^]]*]"""), " ")
            .replace(
                Regex("""\b(feat|ft|featuring|official|audio|video|lyrics)\b"""),
                " ",
            )
